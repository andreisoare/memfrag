...
typedef
   struct _HP_Chunk {
      ...
      SizeT             life;       // Total time between allocation and deallocation
   }
   HP_Chunk;
                  ...
                  Bool is_zeroed )
{
   ...
   hc->data     = (Addr)p;
   hc->where    = NULL;
   hc->life     = guest_instrs_executed; 
   VG_(HT_add_node)(malloc_list, hc);

   // Make new Allocation_List node, add to the list
   VG_(printf)("m %lu %p\n", req_szB, p);
...
void die_block ( void* p, Bool custom_free )
{
   ...
   VG_(printf)("%lu ", hc->life); //time of allocation

   hc->life = guest_instrs_executed - hc->life; //calculate the lifetime

   sxpt = dup_XTree(hc->where, heap_szB + heap_extra_szB + stacks_szB);
   if (sxpt->Sig.ip == 0) 
       ip_desc = "(heap allocation functions) malloc/new/new[], --alloc-fns, etc.";
   else
       ip_desc = VG_(describe_IP)(sxpt->Sig.ip-1, ip_desc, BUF_LEN);

   VG_(printf)("%lu %lu %s\n", hc->life, heap_szB, ip_desc+10); //print its lifetime, heap size and place of allocation
...
void* renew_block ( ThreadId tid, void* p_old, SizeT new_req_szB )
{
   ...
   VG_(printf)("f %p\n", hc->data);
   VG_(printf)("%lu ",hc->life); //time of the allocation
   ...
   VG_(printf)("%lu %lu %s\n", hc->life, heap_szB, ip_desc+11); //print its lifetime and place
